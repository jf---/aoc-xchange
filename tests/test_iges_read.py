...
def test_iges_importer_happy_path():
    r"""happy path"""
    importer = aocxchange.iges.IgesImporter(aocxchange.utils.path_from_file(__file__,
...
def test_iges_importer_happy_topology():
    ...
    compound = importer.compound
    assert len(importer.faces) == 6

    topo = aocutils.topology.Topo(compound)
    assert isinstance(importer.shell, OCC.TopoDS.TopoDS_Shell)
    assert isinstance(importer.solid, OCC.TopoDS.TopoDS_Solid)
...
def test_iges_importer_2_boxes():
    ...
    assert isinstance(importer.shell, OCC.TopoDS.TopoDS_Shell)  # only 1 shell made of 12 faces !!
    assert isinstance(importer.solid, OCC.TopoDS.TopoDS_Solid)
    assert len(importer.faces) == 6 * 2
    compound = importer.compound
    topo = aocutils.topology.Topo(compound)
