...
class StepXcafImporter(object):
    ...
    def getName(self, label):
        # Get the part name
        h_name = TDataStd.Handle_TDataStd_Name()
        label.FindAttribute(TDataStd.TDataStd_Name_GetID(), h_name)
    ...
    def getColor(self, shape):
        ...
        self.color_tool.GetObject().GetColor(shape,
                                             XCAFDoc.XCAFDoc_ColorSurf, color)
        ...
        for j in range(comps.Length()):
            ...
            if isRef:
                ...
                if self.shape_tool.IsSimpleShape(refLabel):
                    ...
                    rComps = TDF.TDF_LabelSequence()  # Components of Assy
                    subchilds = False
    ...
    def read_file(self):
        ...
        self.color_tool = XCAFDoc.XCAFDoc_DocumentTool().ColorTool(doc.Main())
        self.layer_tool = XCAFDoc.XCAFDoc_DocumentTool().LayerTool(doc.Main())
        ...
        if isAssy:
            ...
            self.assyUidStack.append(newAssyUID)
            topComps = TDF.TDF_LabelSequence()  # Components of Top Assy
            subchilds = False
            ...
            for j in range(labels.Length()):
                ...
                if shapeType == 2:
                    logger.debug("The shape type is TopAbs.TopAbs_SOLID")
                    self.tree.create_node(name,
                                          self.getNewUID(),
                                          self.assyUidStack[-1],
